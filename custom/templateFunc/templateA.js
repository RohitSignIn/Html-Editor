function tempA() {
  ...
  document.querySelector("#temp-wrap-con").style.background = "radial-gradient(circle, rgba(238,174,202,1) 0%, rgba(148,187,233,1) 100%)";
  ...
  for (const child of allDirectChilds) {
    ...
    if (type == "section") {
      ...
      sectionCount += 1
      if(sectionCount % 2 == 0){
        child.style.backgroundColor = tempColorPlt.sectionBg2;
      } else {
        child.style.backgroundColor = tempColorPlt.sectionBg1;
      }
      ...
      for (const secChild of allSectionChild) {
        ...
        secChild.style.justifyContent = "start";
        ...
        const secChilds = [...secChild.children]
        if(secChilds[1] != undefined) {
          secChilds[1].style.minWidth = "150px"
        }
      }
  ...
  for (const input of tempWrap.querySelectorAll("input")) {
    ...
    head1.style.color = tempColorPlt.head1Col;
    head1.parentElement.style.marginTop = '0px';
    head1.parentElement.style.marginBottom = '1.5rem';
    head1.parentElement.style.justifyContent = 'center';
    head1.parentElement.style.padding = '0 0.2rem';
    head1.parentElement.style.borderBottom = '1px solid #e7e7e7';
